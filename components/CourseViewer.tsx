...
type Props = {
  ...
  lessonProgress: number[]; // Assuming it stores completed lesson IDs
  setLessonProgress: (lessonProgress: number[]) => void; // Fixed typo
};
...
const CourseViewer = ({
  ...
  const lessonIndex = params.slug?.[2] ? parseInt(params.slug[2]) - 1 : 0;
